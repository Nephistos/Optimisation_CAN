...
'suppression des non numériques
    ...
    'balayage des lignes de isr synthese
     For i = 2 To VRange_isr_synthese.Rows.Count
         ...
         For Caractere = 1 To Len(ThisWorkbook.Worksheets("ISR_Synthese").Cells(i, 31))
             'si detection des numeriques...
             If IsNumeric(Mid(ThisWorkbook.Worksheets("ISR_Synthese").Cells(i, 31), Caractere, 1)) Then
                'stockage des numeriques dans la variable intermediaire
                Chaine = Chaine & Mid(ThisWorkbook.Worksheets("ISR_Synthese").Cells(i, 31), Caractere, 1)
             'sinon si on a un espace on en tient pas compte
             ElseIf (Mid(ThisWorkbook.Worksheets("ISR_Synthese").Cells(i, 31), Caractere, 1) = " ") Then Caractere = Caractere
             'sinon (autre que numeriques et espace...) on met notre compteur de caractère à la taille maximale de la cellule
             Else: Caractere = Len(ThisWorkbook.Worksheets("ISR_Synthese").Cells(i, 31))
             End If
         Next Caractere
         ...
         If ((ThisWorkbook.Worksheets("ISR_Synthese").Cells(i, 40).Value = "") And (ThisWorkbook.Worksheets("ISR_Synthese").Cells(i, 31).Value <> "")) Then
