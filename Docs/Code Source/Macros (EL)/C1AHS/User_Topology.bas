...
For pointeur4 = 2 To VRange_user_topology.Rows.Count
    ...
    For pointeur5 = 10 To VRange_frame_synthese.Columns.Count
        'si on trouve que les calculateurs étudiés sont identitiques alors...
        If (ThisWorkbook.Worksheets("User Topology").Cells(pointeur4, 1).Value = ThisWorkbook.Worksheets("Frame Synthesis (FD+HS) all CAN").Cells(1, pointeur5).Value) Then
