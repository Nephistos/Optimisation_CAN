...
For pointeur1 = 5 To VRange_frame.Rows.Count
    ...
    For pointeur2 = 2 To VRange_rout_av.Rows.Count
        ...
        If (ThisWorkbook.Worksheets("Network Path").Cells(pointeur2, 1).Value = ThisWorkbook.Worksheets("Frame").Cells(pointeur1, 1).Value) Then
            ...
            For pointeur3 = 5 To VRange_connection.Columns.Count + 5 - 1
                'si la frame passe sur un réseau alors...
                If (ThisWorkbook.Worksheets("Network Path").Cells(pointeur2, pointeur3).Value = "x") Then
                    'on le note dans un tableau
                    reseaux(pointeur3 - 5) = True
                End If
            Next pointeur3
    ...
    For pointeur4 = VRange_frame.Columns.Count - VRange_connection.Columns.Count + 1 + 2 To VRange_frame.Columns.Count
        ...
        If (reseaux(pointeur4 - VRange_frame.Columns.Count + VRange_connection.Columns.Count - 1 - 2) = True) Then
            'si c'est un réseau CAN
            If (ThisWorkbook.Worksheets("Frame").Cells(2, pointeur4) = "0") Then
                'on impute sa charge au réseau avec a formule pour le CAN
                ThisWorkbook.Worksheets("Frame").Cells(pointeur1, pointeur4).Value = (((34 + 8 * ThisWorkbook.Worksheets("Frame").Cells(pointeur1, 4).Value) * 1.1 + 13) * (1 / (ThisWorkbook.Worksheets("Frame").Cells(pointeur1, 3).Value * 10 ^ -3))) / 500000
            'sinon on fait la même chose avec la formule pour le CAN FD
            Else: ThisWorkbook.Worksheets("Frame").Cells(pointeur1, pointeur4).Value = (((16.5 * 1.1 + 12.5) / 500000) + (((10 + 8 * ThisWorkbook.Worksheets("Frame").Cells(pointeur1, 4).Value) * 1.1 + 22) / 2000000)) * (1 / (ThisWorkbook.Worksheets("Frame").Cells(pointeur1, 3).Value * 10 ^ -3))
            End If
        Else:
...
For pointeur7 = 5 To VRange_frame.Rows.Count
    'reinitialisation de notre variable intermédiaire
    Chaine = ""
