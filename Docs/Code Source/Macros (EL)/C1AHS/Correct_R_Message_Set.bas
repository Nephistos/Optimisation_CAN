...
'copie des lignes CRC et clock dans Param (avec le bon update time)
    ...
    For pointeur3 = 2 To VRange_r_table.Rows.Count
        If (ThisWorkbook.Worksheets("R_Table").Cells(pointeur3, 5).Interior.Color = RGB(0, 255, 0)) Then
            Worksheets("R_Table").Cells(pointeur3, 5).EntireRow.Copy Destination:=Worksheets("Param").Cells(Worksheets("R_Table").Cells(pointeur3, 100).Value, 1)
            Worksheets("Param").Cells(Worksheets("R_Table").Cells(pointeur3, 100).Value, 8).Value = 1
        End If
    Next pointeur3
...
'On fixe tous les paramètres des trames qui contiennent un ou plusieurs ASILs
    ...
    'on supprime la colonne 70 de param
     ThisWorkbook.Worksheets("Param").Cells(1, 100).EntireColumn.Delete
...
For pointeur3 = 2 To VRange_r_table.Rows.Count
    'si on es pas sur un ASIL (déja recopiée...) alors...
    If (Worksheets("R_Table").Cells(pointeur3, 5).Interior.Color <> RGB(0, 255, 0)) Then
